...
class PoeDriver_microsemi_multi_chip_pd69200(object):
    _last_send_key = None
    ...
    def _xmit(self, msg, delay, chip_index=0):
        if len(msg) != POE_PD69200_MSG_LEN:
            raise RuntimeError("Invalid POE Tx message Length: %d" % len(msg))
        self.plat_poe_write(msg, delay, chip_index)

    def _recv(self, chip_index=0):
        return self.plat_poe_read(chip_index)
    ...
    def _communicate(self, tx_msg, delay, chip_index=0):
        retry = 0
        ex="Unknown"
        while retry < POE_PD69200_COMM_RETRY_TIMES:
            try:
                self._xmit(tx_msg, delay, chip_index)
                if retry > 0:
                    print_stderr("Send(retry: {0}): {1}".format(retry, conv_byte_to_hex(tx_msg)))
                rx_msg = self._recv(chip_index)
    ...
    def _run_communication_protocol(self, command, delay, chip_index=0, msg_type=None):
        tx_msg = self._build_tx_msg(command)
        ...
        rx_msg = self._communicate(tx_msg, delay, chip_index)
    ...
    def reset_poe(self, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   ...
                   0x00,
                   POE_PD69200_MSG_SUB1_RESET]
        return self._run_communication_protocol(command, self._reset_poe_chip_delay, chip_index,
                                                PoeMsgParser.MSG_CMD_STATUS)

    def restore_factory_default(self, chip_index=0):
        ...
        return self._run_communication_protocol(command, self._restore_factory_default_delay, 
                                                chip_index, PoeMsgParser.MSG_CMD_STATUS)

    def save_system_settings(self, chip_index=0):
        command = [POE_PD69200_MSG_KEY_PROGRAM,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_E2,
                   POE_PD69200_MSG_SUB1_SAVE_CONFIG]
        return self._run_communication_protocol(command, self._save_sys_delay, chip_index,
                                                PoeMsgParser.MSG_CMD_STATUS)

    def set_user_byte_to_save(self, user_val, chip_index=0):
        command = [POE_PD69200_MSG_KEY_PROGRAM,
                   self._calc_msg_echo(),
    ...
    def set_system_status(self, priv_label, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
                   POE_PD69200_MSG_SUB1_SYSTEM_STATUS,
                   priv_label]
        return self._run_communication_protocol(command, self._msg_delay, chip_index,
                                                PoeMsgParser.MSG_CMD_STATUS)

    def get_system_status(self, chip_index=0):
    ...
    def get_bt_system_status(self, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
    ...
    def set_individual_mask(self, mask_num, enDis, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
    ...
    def get_individual_mask(self, mask_num, chip_num=2):
        ...
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   ...
                   POE_PD69200_MSG_SUB1_INDV_MSK,
                   mask_num]
        for chip_index in range(chip_num):
            recv_data = self._run_communication_protocol(command, self._msg_delay, chip_index,
                                                         PoeMsgParser.MSG_INDV_MASK)
            combine_val += (recv_data['enDis'] << (chip_index*4))
        ret_data.setdefault('enDis', combine_val)
        return ret_data

    def get_software_version(self, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
    ...
    def support_4wire_bt(self, min_major_ver=3, chip_index=0):
        poe_ver = self.get_poe_versions(chip_index)
    ...
    def set_temp_matrix(self, logic_port, phy_port_a, phy_port_b=0xFF, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
    ...
    def get_temp_matrix(self, logic_port, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
                   POE_PD69200_MSG_SUB1_TEMP_MATRIX,
    ...
    def program_active_matrix(self, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
                   POE_PD69200_MSG_SUB1_TEMP_MATRIX]
    ...
    def get_active_matrix(self, logic_port, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
    ...
    def set_port_enDis(self, logic_port, EnDis, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
    ...
    def set_bt_port_enDis(self, logic_port, EnDis, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
        ...
        for chip_index in range(chip_num):
            if self._4wire_bt == 0:
                command = [POE_PD69200_MSG_KEY_REQUEST,
                        ...
                        POE_PD69200_MSG_SUB_GLOBAL,
                        POE_PD69200_MSG_SUB1_EN_DIS]
                all_ports_endis += self._run_communication_protocol(command, self._msg_delay,
                                                                    chip_index, PoeMsgParser.MSG_ALL_PORTS_ENDIS)
                return all_ports_endis
            else:
    ...
    def set_port_power_limit(self, logic_port, power_limit, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
    ...
    def get_port_power_limit(self, logic_port, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
                   POE_PD69200_MSG_SUB1_SUPPLY,
    ...
    def set_port_priority(self, logic_port, priority, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
    ...
    def set_bt_port_priority(self, logic_port, priority, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
                   POE_PD69200_BT_MSG_SUB1_PORTS_PARAMETERS,
    ...
    def get_port_priority(self, logic_port, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
                   POE_PD69200_MSG_SUB1_PRIORITY,
    ...
    def get_port_status(self, logic_port, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
    ...
    def set_pm_method(self, pm1, pm2, pm3, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
                   POE_PD69200_MSG_SUB1_SUPPLY,
    ...
    def get_pm_method(self, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
                   POE_PD69200_MSG_SUB1_SUPPLY,
    ...
    def get_total_power(self, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
                   POE_PD69200_MSG_SUB1_SUPPLY,
    ...
    def set_power_bank(self, bank, power_limit, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
                   POE_PD69200_MSG_SUB1_SUPPLY,
    ...
    def get_power_bank(self, bank, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
                   POE_PD69200_MSG_SUB1_SUPPLY,
    ...
    def get_power_supply_params(self, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
                   POE_PD69200_MSG_SUB1_SUPPLY,
    ...
    def get_port_measurements(self, logic_port, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
    ...
    def get_bt_port_measurements(self, logic_port, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
    ...
    def get_poe_device_parameters(self, csnum, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_GLOBAL,
    ...
    def get_poe_versions(self, chip_index=0):
        versions = self.get_software_version(chip_index)
    ...
    def get_shutdown_voltage(self, chip_index=0, msg_type=0):
        params = self.get_power_supply_params(chip_index)
        if msg_type == 0:
            return params.get(MAX_SD_VOLT)
        elif msg_type == 1:
            return params.get(MIN_SD_VOLT)
        else:
            return None

    def get_current_power_bank(self, chip_index=0):
        params = self.get_power_supply_params(chip_index)
    ...
    def get_bt_port_parameters(self, logic_port, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
                   POE_PD69200_BT_MSG_SUB1_PORTS_PARAMETERS,
    ...
    def get_bt_port_class(self, logic_port, chip_index=0):
        command = [POE_PD69200_MSG_KEY_REQUEST,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
    ...
    def set_bt_port_operation_mode(self, logic_port, mode, chip_index=0):
        command = [POE_PD69200_MSG_KEY_COMMAND,
                   self._calc_msg_echo(),
                   POE_PD69200_MSG_SUB_CHANNEL,
                   POE_PD69200_BT_MSG_SUB1_PORTS_PARAMETERS,
...
class poePort(object):
    ...
    def update_port_status(self):
        chip_index = self.poe_plat.get_port_chip(self.port_id)
        port_index = self.poe_plat.get_logic_port(self.port_id)
        if self._4wire_bt == 1:
            params = self.poe_plat.get_bt_port_parameters(port_index, chip_index)
            params_class = self.poe_plat.get_bt_port_class(port_index, chip_index)
            ...
            priority = self.poe_plat.get_port_priority(port_index, chip_index)
            self.priority = TBL_PRIORITY_TO_CFG[priority.get(PRIORITY)]

            power_limit = self.poe_plat.get_port_power_limit(port_index, chip_index)
            self.power_limit = power_limit.get(PPL)

            meas = self.poe_plat.get_port_measurements(port_index, chip_index)
            self.current = meas.get(CURRENT)
            self.power_consump = meas.get(POWER_CONSUMP)
            self.voltage = meas.get(VOLTAGE)
    ...
    def set_enDis(self, set_val, current_enDis=None, check_Readback=False):
        ...
        if current_enDis != None and ENDIS in current_enDis and \
            ...
            if self._4wire_bt == 1:
                result = self.poe_plat.set_bt_port_enDis(port_index, set_val, chip_index)
                if check_Readback:
                    result_get = self.poe_plat.get_bt_port_parameters(port_index, chip_index).get(ENDIS)
                if result == 0 or result_get == set_val:
                    ret_flag = result
            else:
                result = self.poe_plat.set_port_enDis(port_index, set_val, chip_index)
                if check_Readback:
                    result_get = self.poe_plat.get_port_status(
                        self.port_id, chip_index).get(ENDIS)
                if result == 0 or result_get == set_val:
                    ret_flag = result
    ...
    def set_powerLimit(self, set_val, check_Readback=False):
        ...
        if self._4wire_bt == 1:
            ...
            result = self.poe_plat.set_port_power_limit(port_index, set_val, chip_index)
            if check_Readback:
    ...
    def set_priority(self, set_val, check_Readback=False):
        ...
        port_index = self.poe_plat.get_logic_port(self.port_id)
        if self._4wire_bt == 1:
            result = self.poe_plat.set_bt_port_priority(port_index, set_val, chip_index)
            if check_Readback:
                result_get = self.poe_plat.get_bt_port_parameters(port_index, chip_index).get(PRIORITY)
            if result == 0 or result_get == set_val:
                ret_flag = result
        else:
            result = self.poe_plat.set_port_priority(port_index, set_val, chip_index)
            if check_Readback:
                result_get = self.poe_plat.get_port_priority(
                    self.port_id, chip_index).get(PRIORITY)
            if result == 0 or result_get == set_val:
                ret_flag = result
        return ret_flag
    ...
    def set_all_params(self, params, current_enDis=None, check_Readback=False):
        ...
        if ENDIS in params:
            ...
            ret_flag[ENDIS] = self.set_enDis(set_val, current_enDis, check_Readback)
...
class poeSystem(object):
    def __init__(self, poe_plat):
        self.poe_plat = poe_plat
        self.chip_num = self.poe_plat.poe_chip_num()
        self.total_ports = 0
        self.total_power = 0
        self.power_consump = 0
        self.power_avail = 0
    ...
    def update_system_status(self, chip_index=0):
        params = self.poe_plat.get_power_supply_params(chip_index)
        ...
        self.power_src += self.poe_plat.bank_to_psu_str(self.power_bank, chip_index)
        if self._4wire_bt == 1:
            system_status = self.poe_plat.get_bt_system_status(chip_index)
            ...
            system_status = self.poe_plat.get_system_status(chip_index)
            ...
            pm_method = self.poe_plat.get_pm_method(chip_index)
            self.pm1 += pm_method.get(PM1)
            self.pm2 += pm_method.get(PM2)
            self.pm3 += pm_method.get(PM3)

    def get_current_status(self, more_info=True):
        for chip_index in range(self.chip_num):
            self.update_system_status(chip_index)
